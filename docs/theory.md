...
이번 범위는 메시징 운영 심화가 아닙니다. 같은 `orderId`의 중복 알림은 인메모리에서 막고, 영속 멱등성, 재처리, dead letter queue, outbox pattern은 한계로 남깁니다.
...
남은 한계는 명확합니다. 현재 구현은 인메모리에서 `orderId` 중복만 막는 기본 RabbitMQ 흐름입니다. 재시작 후에도 유지되는 멱등성, 재처리, 메시지 유실 대응, dead letter queue, outbox pattern, 분산 트랜잭션은 별도 주제로 남습니다.
